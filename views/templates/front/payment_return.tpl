...
{if $status != "error"}
	<img src="{$this_path|escape:'htmlall':'UTF-8'}/views/img/maksuturva.gif" width="115" height="29" style="float: right;"/>
{/if}
{if $status == 'ok'}
	<p>{l s='Your order on' mod='maksuturva' mod='maksuturva'} <span class="bold">{$shop_name|escape:'htmlall':'UTF-8'}</span> {l s='is complete.' mod='maksuturva'}
...
{elseif $status == 'pending'}
	<p>{l s='Your order on' mod='maksuturva'} <span class="bold">{$shop_name|escape:'htmlall':'UTF-8'}</span> {l s='is pending.' mod='maksuturva'}
...
{elseif $status == 'cancel'}
    <p>{l s='Your order on' mod='maksuturva'} <span class="bold">{$shop_name|escape:'htmlall':'UTF-8'}</span> {l s='has been canceled.' mod='maksuturva'}
        <br /><br />{l s='For any questions or for further information, please contact our' mod='maksuturva'} <a href="{$link->getPageLink('contact-form.php', true)}">{l s='customer support' mod='maksuturva'}</a>.
    </p>
{else}
	<p class="warning">{l s='An error occurred while processing the payment.' mod='maksuturva'}</p>
	<p class="warning" style="font-weight: bold;">{$message|escape:'htmlall':'UTF-8'}</p>
	<p class="warning">
